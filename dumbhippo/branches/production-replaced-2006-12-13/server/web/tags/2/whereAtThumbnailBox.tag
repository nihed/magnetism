	...
	<div class="dh-thumbnail-box">
		<div class="dh-thumbnail-meta">
			...
			<div class="dh-thumbnail-status">
				<c:out value="${account.externalAccount.totalThumbnailItemsString}" />
			</div>
		</div>
		<div class="dh-thumbnail-photos-border">
			<div class="dh-thumbnail-photos">
				<c:forEach items="${account.externalAccount.thumbnails}" end="4" var="thumbnail" varStatus="status">
					<c:choose>
					...
					<div class="dh-thumbnail-photo ${css}">
						<a title="${thumbnail.thumbnailTitle}" href="${thumbnail.thumbnailHref}"><img src="${thumbnail.thumbnailSrc}" width="${account.externalAccount.thumbnailWidth}" height="${account.externalAccount.thumbnailHeight}" /></a>
