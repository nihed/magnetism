...
<%@ attribute name="imageSize" required="false" type="java.lang.Integer" %>

<dh:default var="imageSize" value="30"/>
...
<c:choose>
	<c:when test="${!empty who.homeUrl}">
	   	<jsp:element name="a">
	 		...
	 		<jsp:body><dht3:entityLinkContent who="${who}" imageOnly="${imageOnly}" imageSize="${imageSize}"/></jsp:body>
	   	</jsp:element>
	</c:when>
	<c:otherwise>
		<dht3:entityLinkContent who="${who}" imageOnly="${imageOnly}" imageSize="${imageSize}"/>
	</c:otherwise>
</c:choose>
