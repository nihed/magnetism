...
public class MediaModuleParser implements ModuleParser {
        ...
        try {
            for (int i = 0; (contents != null) && (i < contents.size()); i++) {
                ...
                if (content.getAttributeValue("url") != null) {
                    try{
                        mc = new MediaContent(new UrlReference(
                                new URL(content.getAttributeValue("url"))));
                ...
                try{
                    ...
                    ? null : new Long(content.getAttributeValue("fileSize")));
                } catch (Exception ex) {
                    LOG.log(Level.WARNING, "Exception parsing content tag.", ex);
                }
                try{
                    mc.setFramerate((content.getAttributeValue("framerate") == null)
                    ? null : new Float(content.getAttributeValue("framerate")));
                } catch (Exception ex) {
                    LOG.log(Level.WARNING, "Exception parsing content tag.", ex);
                }
                try{
                    mc.setHeight((content.getAttributeValue("height") == null)
                    ? null : new Integer(content.getAttributeValue("height")));
                } catch (Exception ex) {
                    LOG.log(Level.WARNING, "Exception parsing content tag.", ex);
                }
                ...
                try{
                    ...
                    ? null : new Float(content.getAttributeValue("ramplingrate")));
                } catch (Exception ex) {
                    LOG.log(Level.WARNING, "Exception parsing content tag.", ex);
                }
                ...
                try{
                    mc.setWidth((content.getAttributeValue("width") == null) ? null
                            : new Integer(
                            content.getAttributeValue("width")));
                } catch (Exception ex) {
                    LOG.log(Level.WARNING, "Exception parsing content tag.", ex);
                }
    ...
    private Metadata parseMetadata(Element e) {
        ...
        try {
            ...
            if (copy != null) {
                ...
                ? new URL(copy.getAttributeValue("url")) : null);
            }
        } catch (Exception ex) {
        ...
        // ratings
        {
            ...
            for (int i = 0; (ratings != null) && (i < ratings.size()); i++) {
                try {
                    ...
                    String scheme = rat.getAttributeValue("scheme");
                    if (scheme == null)
                    	scheme = "urn:simple";
                    values.add(new Rating(scheme, rat.getText()));
                } catch (Exception ex) {
    ...
    private PlayerReference parsePlayer(Element e) {
        ...
        if (player != null) {
            ...
            try {
                p = new PlayerReference(new URL(player.getAttributeValue("url")),
                        width, height);
            } catch (Exception ex) {
