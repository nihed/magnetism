...
public class MediaModuleTest extends AbstractTestCase {
    ...
    public void testParse() throws Exception {
        SyndFeedInput input = new SyndFeedInput();
        File test = new File( super.getTestFile( "test") );
        ...
        for( int j=0; j < files.length; j++ ){
            ...
            output.output( feed, new File( super.getTestFile("target/"+j+".xml")) );
            SyndFeed feed2 = input.build( new File( super.getTestFile("target/"+j+".xml") ) );
            for( int i=0; i < entries.size(); i++ ){
                BufferedWriter b = new BufferedWriter( new FileWriter( "target/"+j+"a.txt") );
                b.write( ((SyndEntry) entries.get(i)).getModule( MediaModule.URI ).toString() );
                b.close();
                b = new BufferedWriter( new FileWriter( "target/"+j+"b.txt") );
                b.write( ((SyndEntry) feed2.getEntries().get(i)).getModule( MediaModule.URI ).toString() );
                b.close();
