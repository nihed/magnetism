                                        ...
                                        char**key)
{
    ...
    user_agent_key = get_user_agent_key(r);

    /* Key format is a URI, optionally without the query-string */
    if (conf->ignorequerystring) {
        *key = apr_pstrcat(p, scheme, "://", hostname, port_str,
                           r->parsed_uri.path, "?", "|", user_agent_key, NULL);
    }
    else {
        *key = apr_pstrcat(p, scheme, "://", hostname, port_str,
                           r->parsed_uri.path, "?", r->parsed_uri.query, "|", user_agent_key, NULL);
    }
