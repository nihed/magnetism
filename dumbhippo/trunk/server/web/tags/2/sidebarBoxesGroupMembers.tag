...
<c:if test="${group.viewedGroup.status.participant && group.invitedMembers.size > 0}"> <%-- Allow invitees to see other invitees FIXME having the "access control" check here is wrong, should be in GroupSystem --%>
...
<c:if test="${group.viewedGroup.status.participant && group.invitedFollowers.size > 0}"> <%-- Allow invitees to see other invitees FIXME having the "access control" check here is wrong, should be in GroupSystem --%>
